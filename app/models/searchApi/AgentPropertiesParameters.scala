...
case class AgentPropertiesParameters(
  ...
  def permissionString(agentPermission: AgentPermission): Option[String] =
    if (agentPermission == StartAndContinue) Some(agentPermission.name) else None
