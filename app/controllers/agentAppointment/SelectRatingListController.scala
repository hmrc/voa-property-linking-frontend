...
import controllers.agentAppointment.AppointNewAgentForms.manageMultipleProperties
import form.EnumMapping
import form.Mappings.mandatoryBoolean
import models.{RatingListYears, propertyrepresentation}
import models.propertyrepresentation.{AddAgentOptions, AppointNewAgentSession, ChooseFromList, ManagingProperty, RatingListYearsOptions, SelectedAgent}
...
class SelectRatingListController @Inject()(
  ...
  def show(fromCyaChange: Boolean = false): Action[AnyContent] = authenticated.async { implicit request =>
    ...
    }.flatten else Future.successful(NotFound(errorHandler.notFoundErrorTemplate))
  }
    ...
    implicit request =>
      ratingListYears
        ...
        .fold(
          errors => {
            Future.successful(
              BadRequest(selectRatingListView(fromCyaChange, errors, getBackLink)))
          },
          success => {
            if (fromCyaChange) {
