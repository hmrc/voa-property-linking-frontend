...
class ManageDrafts @Inject()(
  ...
  val draftCaseForm: Form[DeleteDraftCase] = Form(
    mapping(
      "draft" -> optional(text)
        .verifying("error.common.noValueSelected", s => s.isDefined && s.nonEmpty)
  ...
  def viewDraftCases() = authenticated(Redirect(config.dashboardUrl("your-drafts")))

  def continueCheck = authenticated.async { implicit request =>
    draftCaseForm.bindFromRequest
      .fold(getDraftCases, success => Future.successful(Redirect(config.checkUrl + getIdUrl(success.draft)._2)))
  }
  ...
  def confirmDelete(draftId: String) = authenticated.async { implicit request =>
    draftCases
      .delete(draftId)
      .map(_ => Redirect(routes.ManageDrafts.viewDraftCases()))
      .recover {
        case _ => Redirect(routes.ManageDrafts.viewDraftCases())
      }
  }
