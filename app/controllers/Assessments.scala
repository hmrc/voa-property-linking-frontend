...
class Assessments @Inject()(
    ...
    (pLink map {
      ...
      case Some(link) =>
        if (!link.pending && link.assessments.size == 1 && isSkipAssessment) {
          Redirect(
            routes.Assessments.viewDetailedAssessment(
              submissionId,
  ...
  private def getViewSummaryCall(uarn: Long, pending: Boolean, owner: Boolean): Call =
    if (owner) routes.Assessments.viewOwnerSummary(uarn, pending)
    else routes.Assessments.viewClientSummary(uarn, pending)
  ...
  def submitViewAssessment(authorisationId: Long, submissionId: String, owner: Boolean): Action[AnyContent] =
    authenticated.async { implicit request =>
      viewAssessmentForm
        ...
        .fold(
          errors => {
            val pLink =
              if (request.organisationAccount.isAgent) propertyLinks.getClientAssessmentsWithCapacity(submissionId)
              else propertyLinks.getOwnerAssessmentsWithCapacity(submissionId)
                    ...
                    views.html.dashboard.assessments(
                      AssessmentsVM(
                        ...
                        assessmentsWithLinks =
                          link.assessments.map(decideNextUrl(submissionId, authorisationId, _, link.pending, owner)),
