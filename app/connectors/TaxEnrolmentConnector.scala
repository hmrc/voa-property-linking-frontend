...
class TaxEnrolmentConnector @Inject()(wSHttp: WSHttp) extends ServicesConfig {
  ...
  def enrol(personId: Long, postcode: String)(implicit hc: HeaderCarrier, ex: ExecutionContext, request: Request[_]): Future[HttpResponse] = {
  ...
  def updatePostcode(
                    ...
                    (implicit hc: HeaderCarrier, ex: ExecutionContext, request: Request[_]): Future[HttpResponse] = {
      val payload = PayLoad(
      ...
      wSHttp.PUT[PayLoad, HttpResponse](updateUrl(personId), payload).map{ result =>
