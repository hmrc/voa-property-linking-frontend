...
package object resources {
  ...
  } yield GroupAccount(id, groupId, companyName, addressId, randomEmail, phone.mkString, isAgent, Some(agentCode).filter(_ => isAgent))
  ...
  val propertyLinkGen: Gen[PropertyLink] = for {
    ...
    PropertyLink(authorisationId = linkId, submissionId = submissionId, uarn = uarn, address = address.toString, agents = agents)
  }
