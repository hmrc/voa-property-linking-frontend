...
class AppointPropertiesControllerISpec extends ISpecBase with HtmlComponentHelpers {
  ...
  "onSubmit" should {
    ...
    "return 400 Bad_Request when no agent code is supplied" in new AppointSomePropertiesSetup {
      ...
      val res = await(
        ws.url(
            s"http://localhost:$port/business-rates-property-linking/my-organisation/appoint/properties?agentCode=&backLinkUrl=${backLinkUrl.unsafeValue}")
          .withCookies(languageCookie(English), getSessionCookie(testSessionId))
          .withFollowRedirects(follow = false)
          .withHttpHeaders(HeaderNames.COOKIE -> "sessionId", "Csrf-Token" -> "nocheck")
          .post(body = requestBody)
    ...
    "return 404 when agent data is found in cache" in new AppointSomePropertiesSetup {
      ...
      val res = await(
        ws.url(
            s"http://localhost:$port/business-rates-property-linking/my-organisation/appoint/properties?agentCode=$agentCode&backLinkUrl=${backLinkUrl.unsafeValue}")
          .withCookies(languageCookie(English), getSessionCookie(testSessionId))
          .withFollowRedirects(follow = false)
          .withHttpHeaders(HeaderNames.COOKIE -> "sessionId", "Csrf-Token" -> "nocheck")
          .post(body = requestBody))
