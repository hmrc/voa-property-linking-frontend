...
class RequestDetailedValuationISpec extends ISpecBase with HtmlComponentHelpers {
  ...
  val valuationTabInsetLinkTextPubReliefsWelsh =
    "y rhyddhad a pha eiddo sy’n gymwys ar ei gyfer (yn agor mewn tab newydd)"
  ...
  val valuationTabInsetLinkWelshPropertyHref =
  ...
  Seq("062", "226", "227", "303").foreach { scatCode =>
    s"myOrganisationAlreadyRequestedDetailValuation displays the correct content for viewing a draft valuation - English (English Property scatCode $scatCode)" which {
      ...
      "has correct inset text" in {
        document.select(valuationTabInsetText1Selector).text shouldBe valuationTabInsetTextPub1
        document.select(valuationTabInsetText2Selector).text shouldBe valuationTabInsetTextPub2
      }

      s"has correct link text of $valuationTabInsetLinkText" in {
        document.select(valuationTabInsetDownloadTextSelector).text shouldBe valuationTabInsetLinkTextPubEstimator
        document.select(valuationTabInsetDownloadTextSelector).attr("href") shouldBe valuationTabInsetDownloadHref
      }

      s"has the correct link of $valuationTabInsetLinkTextPubReliefs" in {
        document.select(valuationTabInsetReliefsTextSelector).text() shouldBe valuationTabInsetLinkTextPubReliefs
        document.select(valuationTabInsetReliefsTextSelector).attr("href") shouldBe valuationTabInsetReliefsHref
      }
      ...
      s"has a section heading of $englishEstimatorDisabledHeadingText" in {
        document
          .select(englishEstimatorDisabledHeadingSelector)
          .text shouldBe englishEstimatorDisabledHeadingText
      }
    }
    ...
    s"myOrganisationAlreadyRequestedDetailValuation displays the correct content for viewing a draft valuation - Welsh (English Property scatCode $scatCode)" which {
      ...
      "has correct inset text" in {
        document.select(valuationTabInsetText1Selector).text shouldBe valuationTabInsetTextPub1Welsh
        document.select(valuationTabInsetText2Selector).text shouldBe valuationTabInsetTextPub2Welsh
      }

      s"has correct link text of $valuationTabInsetLinkTextPubEstimatorWelsh" in {
        document.select(valuationTabInsetDownloadTextSelector).text shouldBe valuationTabInsetLinkTextPubEstimatorWelsh
        document.select(valuationTabInsetDownloadTextSelector).attr("href") shouldBe valuationTabInsetDownloadHref
      }

      s"has the correct link of $valuationTabInsetLinkTextPubReliefsWelsh" in {
        document.select(valuationTabInsetReliefsTextSelector).text() shouldBe valuationTabInsetLinkTextPubReliefsWelsh
        document.select(valuationTabInsetReliefsTextSelector).attr("href") shouldBe valuationTabInsetReliefsHref
      }
      ...
      s"has a section heading of $englishEstimatorDisabledHeadingText" in {
        document
          .select(englishEstimatorDisabledHeadingSelector)
          .text shouldBe englishEstimatorDisabledHeadingTextWelsh
      }
    }
