...
class ViewDvrValuationISpec extends ISpecBase {
  ...
  val ownerChallengeCase = ChallengeCaseWithAgent(
  ...
  val checksTableSubmittedDateRowTextWelsh = "21 Gorffennaf 2020"
  ...
  val challengesTableSubmittedDateRowTextWelsh = "21 Gorffennaf 2020"
  ...
  val currentRateableValueTextWelsh = "Gwerth ardrethol cyfredol (1 Ebrill 2023 i’r presennol)"
  val insetTextWelsh =
  ...
  val changeSomethingTextWelsh =
    "Os yw manylion yr eiddo yn anghywir, neu os ydych yn credu bod y gwerth ardrethol yn anghywir, mae’n rhaid i chi gwblhau ffurflen achos Gwirio."
  ...
  def setupRequestStubs(userIsAgent: Boolean): StubMapping = {
    ...
    stubFor {
      get(s"/business-rates-challenge/my-organisations/challenge-cases?submissionId=$submissionId")
        .willReturn {
          aResponse.withStatus(OK).withBody(Json.toJson(ownerChallengeCase).toString())
        }
    }
