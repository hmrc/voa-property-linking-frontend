...
class YourAgentsFlagOffISpec extends ISpecBase with HtmlComponentHelpers {
  ...
  val agentNameSelector: Int => String = (row: Int) =>
    s".govuk-table__row:nth-child($row) > .govuk-table__cell:nth-child(1)"
  val assignedPropertiesSelector: Int => String = (row: Int) =>
    s".govuk-table__row:nth-child($row) > .govuk-table__cell:nth-child(2)"
  val actionSelector: Int => String = (row: Int) =>
    s".govuk-table__row:nth-child($row) > .govuk-table__cell:nth-child(3) > a"
  val noAgentsSelector = "#no-agents"

  val backLinkHref = s"/business-rates-dashboard/home"
  val viewHref: Int => String = (agentCode: Int) =>
