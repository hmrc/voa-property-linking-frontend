...
class UploadControllerISpec extends ISpecBase {
  ...
  val otherEvidenceRadioLocator: Int => String = (radioNumber: Int) =>
    s"#upload-evidence-options > div:nth-child($radioNumber) > input"
  val otherEvidenceRadioLabelLocator: Int => String = (radioNumber: Int) =>
    s"#upload-evidence-options > div:nth-child($radioNumber) > label"
  val otherEvidenceSummaryLocator = "#main-content > div > div > div.govuk-error-summary > div > h2"
  val otherEvidenceSummaryMessageLocator =
    "#main-content > div > div > div.govuk-error-summary > div > div > ul > li > a"
  ...
  val evidenceTypeRedirectUrl: String => String = (evidenceType: String) =>
    s"/business-rates-property-linking/my-organisation/claim/property-links/evidence/$evidenceType/upload"
  def UploadResultsRedirectUrl(evidenceType: String) =
    s"/business-rates-property-linking/my-organisation/claim/property-links/evidence/$evidenceType/upload"
  ...
  "UploadController show method" should {
    ...
    "display the correct content in English for an IP (Owner) that is uploading a business rates bill" which {
      ...
      "should cache correct evidence type & link basis" in {
        ...
        await(mockPropertyLinkingSessionRepository.get[LinkingSession])
          .map(_.uploadEvidenceData.linkBasis shouldBe RatesBillFlag)
      }
    }
    ...
    "display the correct content in English for an IP (Occupier) that is uploading a lease" which {
      ...
      "should cache correct evidence type & link basis" in {
        ...
        await(mockPropertyLinkingSessionRepository.get[LinkingSession])
          .map(_.uploadEvidenceData.linkBasis shouldBe OtherEvidenceFlag)
      }
    }
    ...
    "display the correct content in English for an IP (Owner) that is uploading a stamp duty land tax form" which {
      ...
      "should cache correct evidence type & link basis" in {
        await(mockPropertyLinkingSessionRepository.get[LinkingSession])
          .map(_.evidenceType shouldBe Some(StampDutyLandTaxForm))
        await(mockPropertyLinkingSessionRepository.get[LinkingSession])
          .map(_.uploadEvidenceData.linkBasis shouldBe OtherEvidenceFlag)
      }
    }
    ...
    "display the correct content in English for an IP (Owner and occupier) that is uploading a land registry title" which {
      ...
      "should cache correct evidence type & link basis" in {
        await(mockPropertyLinkingSessionRepository.get[LinkingSession])
          .map(_.evidenceType shouldBe Some(LandRegistryTitle))
        await(mockPropertyLinkingSessionRepository.get[LinkingSession])
          .map(_.uploadEvidenceData.linkBasis shouldBe OtherEvidenceFlag)
      }
    }
    ...
    "display the correct content in English for an IP (Occupier) that is uploading a water rate demand" which {
      ...
      "should cache correct evidence type & link basis" in {
        await(mockPropertyLinkingSessionRepository.get[LinkingSession])
          .map(_.evidenceType shouldBe Some(WaterRateDemand))
        await(mockPropertyLinkingSessionRepository.get[LinkingSession])
          .map(_.uploadEvidenceData.linkBasis shouldBe OtherEvidenceFlag)
      }
    }
    ...
    "display the correct content in English for an IP (Owner) that is uploading a utility bill" which {
      ...
      "should cache correct evidence type & link basis" in {
        await(mockPropertyLinkingSessionRepository.get[LinkingSession])
          .map(_.evidenceType shouldBe Some(OtherUtilityBill))
        await(mockPropertyLinkingSessionRepository.get[LinkingSession])
          .map(_.uploadEvidenceData.linkBasis shouldBe OtherEvidenceFlag)
      }
    }
  ...
  "UploadController continue method" should {
    ...
    "return a 303 and redirects the user (Owner) to the 'Upload evidence' page when selecting 'Lease' and clicking 'Continue' on the 'Other evidence' page" which {
      lazy val res = postUploadEvidencePage(
        language = English,
        "OTHER",
        userIsAgent = false,
        errorPage = false,
  ...
  private def postUploadEvidencePage(
        language: Language,
        evidenceType: String,
        userIsAgent: Boolean,
        errorPage: Boolean,
    ...
    val body: JsObject =
      if (errorPage) Json.obj("evidenceType" -> "")
      else Json.obj("evidenceType"           -> JsString(selectedEvidenceType.getOrElse("")))
  ...
  private def otherEvidencePageTests(
        language: Language,
        relationship: String,
        userIsAgent: Boolean,
        fromCya: Boolean,
  ...
  private def commonEvidencePageTests(
        language: Language,
        evidenceType: String,
        userIsAgent: Boolean,
        relationship: String,
    ...
    def getTitleAndHeading(): (String, String) = {
      val heading = (evidenceType, userIsAgent) match {
        ...
        case ("RATES_BILL", true) =>
          if (language == English) ratesBillHeadingTextAgent else ratesBillHeadingTextAgentWelsh
        ...
        case ("SERVICE_CHARGE", false) =>
          if (language == English) serviceChargeHeadingText else serviceChargeHeadingTextWelsh
        case ("SERVICE_CHARGE", true) =>
          if (language == English) serviceChargeHeadingTextAgent else serviceChargeHeadingTextAgentWelsh
        case ("STAMP_DUTY", false) => if (language == English) stampDutyHeadingText else stampDutyHeadingTextWelsh
        case ("STAMP_DUTY", true) =>
          if (language == English) stampDutyHeadingTextAgent else stampDutyHeadingTextAgentWelsh
        case ("LAND_REGISTRY", false) =>
          if (language == English) landRegistryHeadingText else landRegistryHeadingTextWelsh
        case ("LAND_REGISTRY", true) =>
          if (language == English) landRegistryHeadingTextAgent else landRegistryHeadingTextAgentWelsh
        case ("WATER_RATE", false) => if (language == English) waterRateHeadingText else waterRateHeadingTextWelsh
        case ("WATER_RATE", true) =>
          if (language == English) waterRateHeadingTextAgent else waterRateHeadingTextAgentWelsh
        case ("UTILITY_RATE", false) => if (language == English) utilityBillHeadingText else utilityBillHeadingTextWelsh
        case ("UTILITY_RATE", true) =>
          if (language == English) utilityBillHeadingTextAgent else utilityBillHeadingTextAgentWelsh
      }
  ...
  private def commonOtherEvidencePageTests(
        language: Language,
        evidenceType: String,
        userIsAgent: Boolean,
        relationship: String,
  ...
  "UploadController show method displays correct error messages" should {
    ...
    "display the correct content in English for when a upscan return 'REJECTED'" in {
      ...
      document
        .select("#main-content > div > div > div > div > div > ul > li > a")
        .text() shouldBe "The selected file must be a Word doc, Excel sheet, PDF, PNG or JPG"
      document
        .select("#choose-file-error")
        .text() shouldBe "Error: The selected file must be a Word doc, Excel sheet, PDF, PNG or JPG"
    }
    ...
    "display the correct content in Welsh for when a upscan return 'REJECTED'" in {
      ...
      document
        .select("#main-content > div > div > div > div > div > ul > li > a")
        .text() shouldBe "Rhaid i’r ffeil a ddewiswyd fod yn ddogfen Word, taenlen Excel, PDF, PNG neu JPG"
      document
        .select("#choose-file-error")
        .text() shouldBe "Error: Rhaid i’r ffeil a ddewiswyd fod yn ddogfen Word, taenlen Excel, PDF, PNG neu JPG"
    }
    ...
    "display the correct content in English for when a upscan return 'EntityTooLarge'" in {
      ...
      document
        .select("#main-content > div > div > div > div > div > ul > li > a")
        .text() shouldBe "The selected file must be smaller than 10MB"
    ...
    "display the correct content in Welsh for when a upscan return 'EntityTooLarge'" in {
      ...
      document
        .select("#main-content > div > div > div > div > div > ul > li > a")
        .text() shouldBe "Rhaid i’r ffeil a ddewiswyd fod yn llai na 10MB"
