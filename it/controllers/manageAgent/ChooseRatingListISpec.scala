...
class ChooseRatingListISpec extends ISpecBase with HtmlComponentHelpers {
  ...
  val errorTitleTextWelsh = "Gwall: Dewis rhestr ardrethu y gall Test Agent ei gweithredu ar eich rhan - Valuation Office Agency - GOV.UK"
  ...
  val theVoaUpdatesTextWelsh = "Mae’r VOA yn diweddaru gwerthoedd ardrethol ac yn cyhoeddi rhestr ardrethu newydd bob ychydig flynyddoedd."
  ...
  "ChooseRatingListController post method" should {
    "Redirect to the are you sure multiple page if the user has chosen multiple list years" in {
      ...
      res.headers("Location").head shouldBe "/business-rates-property-linking/my-organisation/appoint/ratings-list/are-you-sure-multiple"
