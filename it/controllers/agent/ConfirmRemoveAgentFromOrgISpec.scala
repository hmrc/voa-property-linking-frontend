...
class ConfirmRemoveAgentFromOrgISpec extends ISpecBase with HtmlComponentHelpers {
  ...
  val ifYouWantText = s"If you want the agent to act for you again, you can reappoint them to your account using agent code ${agentCode.toString}."
  ...
  val ifYouWantTextWelsh = s"Os ydych am i’r asiant weithredu ar eich rhan eto, gallwch ei ailbenodi i’ch cyfrif gan ddefnyddio cod asiant ${agentCode.toString}."
  ...
  private def getConfirmRemoveAgentFromOrgPage(language: Language): Document = {
    ...
    val res = await(
      ws.url(s"http://localhost:$port/business-rates-property-linking/my-organisation/manage-agent/remove/from-organisation/confirm")
