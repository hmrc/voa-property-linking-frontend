...
class ShowRemoveAgentFromIpOrganisationISpec extends ISpecBase with HtmlComponentHelpers {
  ...
  val paragraph1TextWelsh = "Ni fyddant bellach yn gallu ychwanegu eiddo at eich cyfrif a gweithredu arnynt ar eich rhan."
  ...
  private def getRemoveAgentQuestionPage(language: Language) = {
    ...
    val res = await(
      ws.url(s"http://localhost:$port/business-rates-property-linking/my-organisation/manage-agent/remove/from-organisation")
