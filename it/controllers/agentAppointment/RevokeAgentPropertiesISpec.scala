...
class RevokeAgentPropertiesISpec extends ISpecBase with HtmlComponentHelpers {
  ...
  val noAddressChoiceAtLabelSelector = "#main-content > div > div > div.govuk-grid-row.govuk-grid-column-full > form > div.govuk-grid-row.govuk-form-group--error.govuk-grid-column-full > span"
  ...
  val errorTitleText = s"Error: Which of your properties do you want to unassign $agentName from? - Valuation Office Agency - GOV.UK"
  ...
  val errorTitleTextWelsh = s"Gwall: O ba eiddo ydych chi am ddadneilltuo $agentName? - Valuation Office Agency - GOV.UK"
  ...
  val clearSearchHref = s"/business-rates-property-linking/my-organisation/revoke/properties?page=1&pageSize=15&agentCode=$agentCode"
  val selectAllHref = "#"
  ...
  private def postRevokeAgentPropertiesPageWithNoSelection(language: Language, multipleProps: Option[Boolean]): Document = {
    ...
    stubFor {
      get(s"/property-linking/my-organisation/agents/$agentCode/property-links?agent=gg-ext-id&sortField=ADDRESS&sortOrder=ASC&startPoint=1&pageSize=15&requestTotalRowCount=false")
        .willReturn {
    ...
    val res = await(
      ws.url(s"http://localhost:$port/business-rates-property-linking/my-organisation/revoke/properties/create?page=1&pageSize=1&agentCode=$agentCode")
  ...
  private def postRevokeAgentPropertiesPageWithEmptyAddress(language: Language, multipleProps: Option[Boolean]): Document = {
    ...
    val res = await(
      ws.url(s"http://localhost:$port/business-rates-property-linking/my-organisation/revoke/properties/filter?page=1&pageSize=15&agentCode=$agentCode")
        .withCookies(languageCookie(language), getSessionCookie(testSessionId))
        .withFollowRedirects(follow = false)
        .withHttpHeaders(HeaderNames.COOKIE -> "sessionId", "Csrf-Token" -> "nocheck")
        .post(body = requestBody)
  ...
  private def getRevokeAgentPropertiesPage(language: Language, multipleProps: Option[Boolean]): Document = {
    ...
    val res = await(
      ws.url(s"http://localhost:$port/business-rates-property-linking/my-organisation/revoke/properties?page=1&pageSize=15&agentCode=$agentCode")
        .withCookies(languageCookie(language), getSessionCookie(testSessionId))
        .withFollowRedirects(follow = false)
  ...
  private def setupStubs(multipleProps: Option[Boolean]): StubMapping = {
    ...
    stubFor {
      get(s"/property-linking/my-organisation/agents/$agentCode/property-links?sortField=ADDRESS&sortOrder=ASC&startPoint=1&pageSize=15&requestTotalRowCount=false")
        .willReturn {
          aResponse.withStatus(OK).withBody(Json.toJson(
            multipleProps match {
              case Some(true) => testOwnerAuthResultMultipleProperty
