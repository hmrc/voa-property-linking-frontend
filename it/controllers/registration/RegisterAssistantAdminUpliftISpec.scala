...
class RegisterAssistantAdminUpliftISpec extends ISpecBase with HtmlComponentHelpers with ListYearsHelpers {
  ...
  val weUseYourText = "We use your organisation details to send you correspondence related to the service and your account."
  ...
  val weUseYourTextWelsh = "Rydym yn defnyddio manylion eich sefydliad i anfon gohebiaeth atoch yn ymwneud â’r gwasanaeth a’ch cyfrif."
  ...
  private def postForm(redirectUrl: String) = {
    ...
    val res = await(ws.url(s"http://localhost:$port/business-rates-property-linking/complete-existing-business-contact-details-uplift")
